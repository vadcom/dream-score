...
public class AppApiController implements AppApi {
    ...
    public ResponseEntity<List<Score>> appSectionGet(@Parameter(in = ParameterIn.PATH, description = "", required=true, schema=@Schema()) @PathVariable("app") String app,
                                                     ...
                                                     @Max(100) @Parameter(in = ParameterIn.QUERY, description = "" ,schema=@Schema(allowableValues={ "100" }, maximum="100", defaultValue="1")) @Valid @RequestParam(value = "position", required = false, defaultValue="0") Integer position,
